...
def _index_outputs(
    ...
    for i, task in scenario.tasks.items():
        for t in scenario.time_slots:
            ...
            key = (i, t)
            if is_active and key not in assignments:
                raise ValueError(f"active task ({i}, {t}) not assigned")
            if not is_active and key in assignments:
