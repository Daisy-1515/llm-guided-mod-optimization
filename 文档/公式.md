...
由于时延、终端侧能耗、边缘计算能耗与无人机飞行能耗的量纲和量级不同，系统成本建模为加权归一化和。为避免在任务双重求和中重复累计无人机飞行能耗，将飞行能耗单独拆分为独立项。终端设备 $i$ 在时隙 $t$ 将任务卸载到节点 $j\in\mathcal{J}$ 时的传输能耗定义为：

$$
E_{i,j}^{\mathrm{tx},t} = P_i^t T_{i,j}^{\mathrm{up},t}
= \frac{P_i^t D_i^l}{r_{i,j}^t}, \quad j\in\mathcal{J}
$$
...
\right)
}_{\boxed{\text{非凸（含 } 1/r_{i,j}^t \text{，依赖 } \mathbf{q}_j^t\text{）}}}
+ \underbrace{
...
- $E_{i,j}^{\mathrm{tx},t}$：终端设备 $i$ 在时隙 $t$ 将任务上传到节点 $j$ 的传输能耗；
- $E_j^{\mathrm{fly},t}$：无人机 $j$ 在时隙 $t$ 的飞行能耗，该项单独计入一次，不参与任务双重求和；
- $E_i^{\max}, E_j^{\max}$：分别为终端设备 $i$ 与边缘节点 $j$ 的能耗归一化基准或能耗上限；
- $\alpha,\beta,\gamma,\lambda \ge 0$：分别为时延成本、终端侧能耗成本、边缘计算能耗成本与无人机飞行能耗成本的权重系数；
...
1. 离散决策变量：
   ...
   维度上，本地执行变量规模为 $|\mathcal{I}|\times|\mathcal{T}|$，远程卸载变量规模为 $|\mathcal{I}|\times|\mathcal{J}|\times|\mathcal{T}|$。

2. 资源分配变量：
   $\{f_i^t,\ f_{j,i}^t,\ P_i^t \mid i\in\mathcal{I},\ j\in\mathcal{J},\ t\in\mathcal{T}\}$。
   其中，$f_i^t$ 表示设备 $i$ 在时隙 $t$ 的本地 CPU 频率，$f_{j,i}^t$ 表示边缘节点 $j$ 在时隙 $t$ 分配给任务 $U_i^t$ 的 CPU 频率，$P_i^t$ 表示设备 $i$ 在时隙 $t$ 的上行发射功率。
   维度上，$f_i^t$ 与 $P_i^t$ 的规模均为 $|\mathcal{I}|\times|\mathcal{T}|$，$f_{j,i}^t$ 的规模为 $|\mathcal{J}|\times|\mathcal{I}|\times|\mathcal{T}|$。

3. UAV 轨迹与移动变量：
   $\{\mathbf{q}_j^t,\ \mathbf{v}_j^t \mid j\in\mathcal{U},\ t\in\mathcal{T}\}$。
   其中，$\mathbf{q}_j^t=[x_j^t,y_j^t]^T\in\mathbb{R}^2$ 表示无人机 $j$ 在时隙 $t$ 的二维水平位置，$\mathbf{v}_j^t\in\mathbb{R}^2$ 表示其速度向量，且 $v_j^t=\|\mathbf{v}_j^t\|$。
   维度上，位置变量与速度变量的规模均为 $2\times|\mathcal{U}|\times|\mathcal{T}|$（按二维分量计）。
...
flowchart TD
    ...
    C{"执行位置决策"}
    X["离散决策变量<br/>x<sub>i,i</sub><sup>t</sup>, x<sub>i,j</sub><sup>t</sup>"]
    Y["资源分配变量<br/>f<sub>i</sub><sup>t</sup>, f<sub>j,i</sub><sup>t</sup>, P<sub>i</sub><sup>t</sup>"]
    Z["UAV 轨迹变量<br/>q<sub>j</sub><sup>t</sup>, v<sub>j</sub><sup>t</sup>"]
    ...
    C -->|UAV j∈U| U1["上行时延<br/>T<sub>i,j</sub><sup>up,t</sup>=D<sub>i</sub><sup>l</sup>/r<sub>i,j</sub><sup>t</sup>"]
    B --> U1
    U1 --> TX["传输能耗<br/>E<sub>i,j</sub><sup>tx,t</sup>=P<sub>i</sub><sup>t</sup>T<sub>i,j</sub><sup>up,t</sup>"]
    ...
    U2 --> E["边缘计算能耗<br/>E<sub>j,i</sub><sup>comp,t</sup>"]
